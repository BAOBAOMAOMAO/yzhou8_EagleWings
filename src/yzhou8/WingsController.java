...
public class WingsController extends SolitaireReleasedAdapter{
	/** The EagleWings Game. */
	 ...
	 */
	public WingsController(EagleWings theGame, PileView wing) {
		super(theGame);
	...
	public void mousePressed(MouseEvent me ) {
		...
		Pile wings = (Pile) src.getModelElement();
		if (wings.empty()) {
		...
		if (w != Container.getNothingBeingDragged()) {
			System.err.println ("WingsController::mousePressed(): Unexpectedly encountered a Dragging Object during a Mouse press.");
			return;
		}
	...
	public void mouseReleased(MouseEvent me) {
		...
		if (draggingWidget == Container.getNothingBeingDragged()) {
			System.err.println ("WingsController::mouseReleased() unexpectedly found nothing being dragged.");
			c.releaseDraggingObject();		
			return;
		}
		...
		if (fromWidget == null) {
			System.err.println ("WingsController::mouseReleased(): somehow no dragSource in container.");
			c.releaseDraggingObject();
			return;
		}
		...
		if (fromWidget instanceof BuildablePileView) {
			...
			Move move = new WasteToWingsMove(wastePile,theCard,wing);
